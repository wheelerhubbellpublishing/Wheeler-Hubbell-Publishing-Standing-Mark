...
function agentCard(service) {
  return {
    ...
    skills: Object.values(service.products).map((product) => ({
      id: `whp-${product.id}`,
      name: product.name,
...
function a2aMessage(service, requestBody) {
  ...
  const offers = Object.values(service.products).map((product) => ({
    id: product.id,
    name: product.name,
  ...
  return {
    ...
    result: {
      ...
      role: "agent",
      parts: [
        { kind: "text", text: "WHP offers a 0.05 USDC readiness check and a 25 USDC integrity snapshot for one public HTTPS URL. Neither is WHP Standing or proof of truth." },
        { kind: "data", data: { offers, analysis_performed: false, payment_requested: false } },
      ],
      metadata: { service: "WHP Agent/x402 Integrity Market", offer_only: true },
    },
  };
}
...
function openapi(service) {
  ...
  const paths = {};
  for (const product of Object.values(service.products)) paths[product.path] = { post: {
  ...
  return {
    openapi: "3.1.0",
    info: { title: "WHP Agent/x402 Integrity Market", version: "1.0.0", description: "Paid deterministic public-interface observations. Not WHP Standing or proof of truth." },
...
function llms(service) {
  const cardCheckout = service.stripe?.checkoutUrl
    ...
    : "";
  return `# WHP Agent/x402 Integrity Market
...
${cardCheckout}- OpenAPI: ${service.origin}/openapi.json
...
Neither product is WHP Standing, a Standing Mark, proof of truth, authority, security certification, or legal advice. Payment purchases the bounded output, not a favorable finding.
`;
}
...
async function discoveryRoute(service, request, pathname) {
  ...
  if (pathname === "/healthz") {
    ...
    body = canonical({ status: "ok", service: "WHP Agent/x402 Integrity Market", version: "1.0.0", database: "ready", settlement_gate: "finalized-on-chain" });
  }
  ...
  else if (pathname === "/.well-known/x402") body = canonical({
    version: 1,
    name: "WHP Agent/x402 Integrity Market",
    ...
    const card = service.stripe?.checkoutUrl
      ? `<p><a rel="noopener noreferrer" href="${escapeHtml(service.stripe.checkoutUrl)}">Buy the 25 USD snapshot by card</a></p>`
      : "";
    body = `<!doctype html><html lang="en"><meta charset="utf-8"><title>WHP Agent/x402 Integrity Market</title><body><h1>WHP Agent/x402 Integrity Market</h1><p>0.05 USDC readiness checks and 25 USDC integrity snapshots for public HTTPS URLs.</p>${card}<p>Not WHP Standing or proof of truth.</p><p><a href="/openapi.json">OpenAPI</a> · <a href="/.well-known/agent-card.json">Agent Card</a> · <a href="/llms.txt">llms.txt</a></p></body></html>`;
    type = "text/html; charset=utf-8";
  } else return null;
  return new Response(head ? null : `${body}\n`, { status: 200, headers: { ...COMMON_HEADERS, "content-type": type } });
}

export function createMarketService({ origin, products, store, rail, engines, stripe = null, clock = () => Math.floor(Date.now() / 1_000) }) {
