...
export function createSnapshotEngine({ fetcher = createSafeFetcher() } = {}) {
  return async function makeSnapshot(targetInput) {
    ...
    const observations = { target: await observe(fetcher, targetUrl.href), discovery: {} };
    for (const [name, path] of DISCOVERY_PATHS) {
      const url = new URL(path, origin).href;
      observations.discovery[name] = url === targetUrl.href ? observations.target : await observe(fetcher, url);
    }
