...
export function runtimeConfiguration(env = process.env) {
  ...
  const facilitatorRaw = env.WHP_FACILITATOR_URL || "";
  const rpcRaw = env.WHP_RPC_URL || "";
  demand(Boolean(facilitatorRaw) === Boolean(rpcRaw), "CONFIG_X402_INCOMPLETE", 503);
  const facilitatorUrl = facilitatorRaw
    ? httpsUrl(facilitatorRaw, "CONFIG_WHP_FACILITATOR_URL_INVALID")
    : null;
  const rpcUrl = rpcRaw ? httpsUrl(rpcRaw, "CONFIG_WHP_RPC_URL_INVALID") : null;
  ...
  demand(Boolean(facilitatorUrl) || Boolean(stripeSecret), "CONFIG_PAYMENT_RAIL_REQUIRED", 503);
  if (stripeSecret) {
  ...
  return {
    ...
    facilitatorUrl,
    rpcUrl,
    x402Enabled: Boolean(facilitatorUrl && rpcUrl),
    databaseUrl,
    payTo,
    port,
    stripe: stripeSecret ? {
