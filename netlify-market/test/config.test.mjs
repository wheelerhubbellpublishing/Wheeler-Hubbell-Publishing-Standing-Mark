...
import { DEFAULT_PAY_TO, paymentRequirements, runtimeConfiguration } from "../src/market/config.mjs";
...
test("runtime configuration permits Stripe-only mode and requires x402 inputs as a pair", () => {
  assert.throws(() => runtimeConfiguration({}), /CONFIG_WHP_MARKET_ORIGIN_REQUIRED/u);
  const stripeOnly = runtimeConfiguration({
    WHP_MARKET_ORIGIN: "https://market.example",
    DATABASE_URL: "postgresql://user:pass@db.example/market",
    STRIPE_WEBHOOK_SECRET: "whsec_live_secret_123",
    STRIPE_PAYMENT_LINK_ID: "plink_live_snapshot_123",
  });
  assert.equal(stripeOnly.x402Enabled, false);
  assert.throws(() => runtimeConfiguration({
    WHP_MARKET_ORIGIN: "https://market.example",
    DATABASE_URL: "postgresql://user:pass@db.example/market",
  }), /CONFIG_PAYMENT_RAIL_REQUIRED/u);
  assert.throws(() => runtimeConfiguration({
    WHP_MARKET_ORIGIN: "https://market.example",
  ...
  const paired = runtimeConfiguration({
    WHP_MARKET_ORIGIN: "https://market.example",
    WHP_FACILITATOR_URL: "https://facilitator.example",
    WHP_RPC_URL: "https://rpc.example/base",
    DATABASE_URL: "postgresql://user:pass@db.example/market",
  });
  assert.equal(paired.x402Enabled, true);
});
